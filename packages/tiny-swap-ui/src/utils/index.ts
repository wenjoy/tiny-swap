...
export async function getFactoryContractWithSigner() {
  ...
  const contractWithSigner = (await contract.connect(
    signer
  )) as ethers.Contract;
  return contractWithSigner;
}
...
export async function calculateMinTokenAmountForLiquidity(
  token0: TOKEN,
  token1: TOKEN,
  tokenAmount: string
): Promise<string> {
  ...
  if (await isFirstDeposit(pair)) {
    ...
    if (isInputToken0) {
      anotherTokenAmount =
        (Number(tokenAmount) * Number(reserve1)) / Number(reserve0);
    } else {
      anotherTokenAmount =
        (Number(tokenAmount) * Number(reserve0)) / Number(reserve1);
    }
...
export async function burn(pair: address, to: address) {
  ...
  const transferResult = await pairContract.transferFrom(
    signer,
    pair,
    liquidity
  );
...
export async function getReserves(pair: address) {
  ...
  return reserves.map((reserve: number) =>
    toPrecision(parseFloat(ethers.formatUnits(reserve, decimals)))
  );
}

export async function withDrawToken0(
  pair: address,
  token: TOKEN,
  to: address,
  value: string
  ...
  const pairContract = await getContractWithSigner(pair, pairABI.abi);
  const tokenDecimal = await tokenDecimals(token);
  const result = await pairContract.withdrawToken0(
    to,
    ethers.parseUnits(value, tokenDecimal)
  );
  return result.wait();
}

//TOKEN specific
export async function isNotSufficient(
  token0: TOKEN,
  token1: TOKEN,
  token0Value: string,
  token1Value: string
  ...
  return (
    Number(balance0) < Number(token0Value) ||
    Number(balance1) < Number(token1Value)
  );
}
